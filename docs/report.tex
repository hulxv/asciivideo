...
\newpage
As you notice, we set the streams during this initilization using $video\_set\_streams$, that making a loop to find the video and audio streams
...
Finally, We will need to decode the video. For that, We make a function that deocde it with passing a hundler that handles the decoding operation
\begin{lstlisting}[language=c]
...
We will start with defining a struct for that that contains the video source, and output directory, and the video struct to decode it.
\begin{lstlisting}[language=c]
typedef struct
{
    const char  *src;
...
Alright, We converted the video into ASCII. Now, We need to hear something. For that, the Audio extraction is important to play the sound with the video. This happens using $FFmpeg$ tool by run it as child process.
\begin{lstlisting}[language=c]
